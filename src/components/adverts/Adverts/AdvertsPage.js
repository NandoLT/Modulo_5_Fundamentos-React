...
const AdvertsPage = ({history, location, ...props }) => {
    ...
    const handleFilter = (dataFilters) => { 
        ...
        const productsFiltered = adverts.filter( advert => {
                if(
                    ...
                    &&
                    (((dataFilters.sale !== null) && (advert.sale === !dataFilters.sale)) || dataFilters.sale === null )
                    &&
    ...
    return (
        ...
        <AdvertFilter onFilter={handleFilter}></AdvertFilter>
            {adverts.length ? <AdvertsList adverts={advertsFiltereds.length === 0 ? adverts : advertsFiltereds} history={history}/> : <EmptyList />}
        </Layout>
    );
};
